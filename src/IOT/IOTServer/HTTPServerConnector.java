...
public class HTTPServerConnector extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        System.out.println("HTTPServerConnector: new subscription request from " + request.getRemoteAddr() + ":" + request.getServerPort() + "!");
        server.subscribeRequestHandler(request.getRemoteAddr(),request.getServerPort()); // send it via servlet /iot (8080 is tomcat port)
        response.setStatus(HttpServletResponse.SC_OK);
